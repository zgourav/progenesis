  ...
  return (
    <section className="section-spacing px-4 sm:px-6 md:px-10 lg:px-14 xl:px-20 py-8 md:py-12">
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between" style={{paddingBottom:'16px'}}>
          <div>
            <span className="inline-block bg-[#E9F0FF] text-[#1656A5] text-[12px] md:text-[13px] px-3 py-1 rounded-[8px]" style={{padding:'8px'}}>First Person. Real Stories.</span>
          ...
          <button style={{padding:'8px'}} className="w-auto h-[32px] inline-flex items-center px-4 py-2 rounded-[8px] bg-blue-600 text-white text-sm shadow-sm hover:bg-blue-700 mt-3 md:mt-0 md:ml-auto">
            See all
          </button>
        </div>
        ...
        <div className="mt-6 flex gap-4 overflow-x-auto md:grid md:grid-cols-2 lg:grid-cols-3 md:gap-6 md:overflow-visible">
          {stories.map((s, i) => (
            <article
              key={i}
              style={{padding:'8px'}}
              className="rounded-2xl border border-gray-200 overflow-hidden shadow-[0_1px_2px_rgba(0,0,0,0.04)] w-[60%] flex-none md:w-auto"
            >
